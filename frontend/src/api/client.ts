...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://netra-ai-y7l0.onrender.com'
...
export const api = {
  ...
  analyzePcap: async (file: File): Promise<OfflineAnalysisResult> => {
    const formData = new FormData()
    formData.append('file', file)
    const response = await apiClient.post<OfflineAnalysisResult>('/analyze-pcap', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    return response.data
  },
  analyzeFile: async (file: File): Promise<OfflineAnalysisResult> => {
    const formData = new FormData()
    formData.append('file', file)
    const response = await apiClient.post<OfflineAnalysisResult>('/analyze-file', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    return response.data
  },
...
export const getBenchmarks = api.getBenchmarks
export const analyzeFlow = api.analyzeFlow
export const analyzePcap = api.analyzePcap
export const analyzeFile = api.analyzeFile
