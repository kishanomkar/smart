import {
  require_jsx_runtime
} from "./chunk-OQQYGHJS.js";
import {
  require_react
} from "./chunk-TAJOU4CS.js";
import {
  __export,
  __toESM
...
function calcGeneratorDuration(generator, timeStep = 50, maxDuration = maxGeneratorDuration, keyframes2) {
  let duration = 0;
...
function findSpring({ duration = springDefaults.duration, bounce = springDefaults.bounce, velocity = springDefaults.velocity, mass = springDefaults.mass }) {
  ...
  if (dampingRatio < 1) {
    ...
    derivative = (undampedFreq2) => {
      ...
      const e = dampingRatio * dampingRatio * undampedFreq2 * undampedFreq2 * duration;
      const f = Math.exp(-delta);
      const g = calcAngularFreq(undampedFreq2 * undampedFreq2, dampingRatio);
...
function animateEffectSubject(effect, subject, keyframes2, transition = {}) {
  const animations2 = [];
...
function animateSubject(subject, keyframes2, options, scope) {
  ...
  if (isSingleValue(subject, keyframes2)) {
    ...
    for (let i = 0; i < numSubjects; i++) {
      ...
      if (effect) {
        animations2.push(...animateEffectSubject(effect, thisSubject, keyframes2, transition));
        continue;
      }
      const createVisualElement2 = isElement ? createDOMVisualElement : createObjectVisualElement;
...
function distanceToBox(point2, box) {
  const x = Math.max(box.x.min - point2.x, 0, point2.x - box.x.max);
  const y = Math.max(box.y.min - point2.y, 0, point2.y - box.y.max);
  return x * x + y * y;
}

// node_modules/framer-motion/dist/es/components/Reorder/utils/detect-axis.mjs
...
function ReorderGroupComponent({ children, as = "ul", axis: axisOverride, onReorder, values, ...props }, externalRef) {
  ...
  const itemLayouts = (0, import_react49.useRef)(/* @__PURE__ */ new Map());
  const [detectedAxis, setDetectedAxis] = (0, import_react49.useState)("y");
...
function ReorderItemComponent({ children, style: style2 = {}, value, as = "li", onDrag, onDragEnd, layout: layout2 = true, ...props }, externalRef) {
  ...
  return (0, import_jsx_runtime11.jsx)(Component3, { drag: dragAxis, ...props, dragSnapToOrigin: true, style: { ...style2, x: point2.x, y: point2.y, zIndex }, layout: layout2, onDrag: (event, gesturePoint) => {
    const { velocity, point: pointerPoint } = gesturePoint;
    const offset = { x: point2.x.get(), y: point2.y.get() };
